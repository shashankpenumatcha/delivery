...
export class CartService {
    ...
    private convertDateFromClient(cart: ICart): ICart {
        const copy: ICart = Object.assign({}, cart, {
            lastUpdated: cart.lastUpdated != null && cart.lastUpdated.isValid() ? cart.lastUpdated.toJSON() : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((cart: ICart) => {
            cart.lastUpdated = cart.lastUpdated != null ? moment(cart.lastUpdated) : null;
        });
        return res;
    }
}
